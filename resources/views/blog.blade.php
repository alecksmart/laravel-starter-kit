            ...
            @foreach ($posts as $post)
                <div class="panel panel-default">
                    <div class="panel-heading">
                        ...
                        <h4><a href="/post/{{ $post->post_slug }}" title="See the full posting with comments">{{ $post->post_title }}</a></h4>
                    </div>
                    <div class="panel-body">
                        @if($post->user->avatar_filename)
                            <p class="pull-right">
                                <img class="img-thumbnail rounded" title="{{ $post->user->name }}" alt="{{ $post->user->name }}" style="width: 100px; height: 100px;" src="/{{ Config::get('constants.AVATAR_WEB_PATH') }}/{{ $post->user->avatar_filename }}">
                            </p>
                        @endif
                        <p>by <em>{{ $post->user->name }}</em>, <a href="mailto:{{ $post->user->email }}">{{ $post->user->email }}</a>:</p>
                        <p>
                            {{ implode(' ', array_slice(explode(' ', $post->post_body), 0, 20)) }} &hellip;
                        </p>
                    </div>
                </div>
            @endforeach
